...
const SERVICES: Service[] = [
	...
	},
	{
		...
		description:
			'Полировка — безопасно и лаконично. Фацет — выразительный акцент.',
	...
	},
	{
		...
		description:
			'Декоративная фаска уже включена в стоимость — без доплаты за кромку.',
	...
	},
	{
		...
		description:
			'Лёгкое и ударостойкое. Хорошо для мест с повышенной безопасностью.',
	},
	{
		id: 'glass_titanium',
		kind: 'glass',
	...
	},
	{
		...
		description:
			'Классическая технология напыления. Хорошее соотношение цена/качество.',
	},
	{
		id: 'glass_silvered',
		kind: 'glass',
...
export default function Pricing() {
	...
	function toggleService(id: ServiceId) {
		setState(prev => {
			...
			if (isGlass(id)) {
				for (const sid of Array.from(nextSel))
					if (isGlass(sid)) nextSel.delete(sid)
			...
			if (id === 'edge' && nextSel.has('edge') && edgeType === 'none')
				edgeType = 'polish'
					...
					<div className='calc-grid'>
						...
						<div className='calc-field'>
							...
							<input
								...
								onChange={e => {
									const v = e.target.value
									setState(p => ({ ...p, height: v === '' ? NaN : Number(v) }))
								}}
								onBlur={e => {
									if (e.target.value === '')
						...
						<div className='calc-field'>
							...
							<div className='helper'>
								Кромка учитывается в расчёте, если включена карточка «Обработка
								кромки».
								{getSelectedGlass(state.selected) === 'glass_facet_incl' &&
						...
						<div className='calc-field'>
							...
							<input
								...
								step={0.1}
								value={
									Number.isFinite(state.edgeMeters) ? state.edgeMeters : ''
								}
								onChange={e => {
									const v = e.target.value
									setState(p => ({
						...
						{region === 'area' && (
							<div className='calc-field'>
								<label className='label'>
									Расстояние от КАД (км, в одну сторону)
								</label>
								<input
									className='input'
									type='number'
									...
									step={1}
									value={
										Number.isFinite(state.kmFromKAD) ? state.kmFromKAD : ''
									}
									onChange={e => {
										const v = e.target.value
										setState(p => ({
